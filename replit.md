...
**November 7, 2025 (Latest)** - **Implemented customizable dashboard for project overview pages**. Added per-project customizable 2x2 dashboard grid allowing users to select which widgets appear in each quadrant. Created 8 widget components: Basic Information, Financial Progress, Progress Overview, Team Members, Action Points, Milestones, Safety Issues, and Recent Updates. Added dashboard_layout JSONB column to projects table storing quadrant preferences with default layout. Built customization modal with 4 dropdowns for widget selection. Refactored overview tab from 688 lines to 114 lines by extracting all display logic into reusable widget components. Added PATCH `/api/projects/:id/dashboard-layout` endpoint for saving preferences. Layout persists across page reloads and is project-specific. Customization button with Settings icon placed in overview page header.
