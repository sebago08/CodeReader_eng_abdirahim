...
**November 6, 2025 (Latest)** - **Migrated authentication from Passport Local to Supabase Auth**. Complete authentication system overhaul: Frontend now uses Supabase Auth SDK for email/password authentication with JWT tokens stored in localStorage. Backend replaced all 93 session-based `isAuthenticated` middleware calls with `supabaseAuthMiddleware` that verifies JWT tokens. Added `authId` field to users table to store Supabase Auth user IDs. Updated frontend auth flow to use email (instead of username), removed `/api/login`, `/api/register`, `/api/logout` Passport endpoints. Backend auto-creates user profiles when users register through Supabase. Query client automatically attaches Bearer tokens to all API requests. Admin routes updated to chain `supabaseAuthMiddleware` before `isAdmin` checks. Session infrastructure remains but is no longer used for authentication. **Note**: Existing users with passwords from old system need to re-register using Supabase Auth.
...
The system uses Supabase Auth with JWT token-based authentication for secure access. Frontend uses Supabase Auth SDK for email/password authentication, with tokens stored in localStorage and automatically attached to all API requests via Authorization headers. Backend verifies JWT tokens using `supabaseAuthMiddleware` which auto-creates user profiles on first login. All users are auto-approved upon registration (no admin approval required). Role-based access control is applied to protected API endpoints, ensuring users only access their own project data. Admin users have additional privileges for system management.

## System Design Choices

The application uses a PostgreSQL database with a normalized schema covering users (with authId for Supabase Auth integration), projects, personnel, equipment, payment certificates, work plans, activities, progress trackers, pre-commencement items, daily logs, action points, roads, construction layers, layer progress, and sessions (legacy). The architecture supports both development (Replit's PostgreSQL, in-memory mock storage) and production (Supabase PostgreSQL, Supabase Storage, Supabase Auth) environments, with environment-based configuration and an abstraction layer for storage services.

# External Dependencies

- **PostgreSQL**: Primary database (Replit's built-in PostgreSQL for development).
- **Supabase**: Provides authentication (JWT tokens), PostgreSQL database (production), and file storage (production). Frontend uses `@supabase/supabase-js` SDK for auth operations.
