...
**November 5, 2025 (Latest)** - Completely redesigned the main dashboard to focus on financial and project health metrics. Replaced the previous action-oriented cards with a cleaner, simplified layout featuring 3 financial metric cards (Financial Total, Amount Spent, Current Balance) and 3 status metric cards (Projects Behind Schedule, Critical Safety Issues with breakdown, Upcoming Milestones). Streamlined the Active Projects table by pre-calculating road-length-weighted progress on the backend, eliminating client-side N+1 queries. Updated DashboardMetrics schema and enhanced the GET /api/dashboard/metrics endpoint to return all data in a single consolidated call, including financial totals derived from active projects only. This improves dashboard load performance and provides immediate visibility into financial status and project health.
