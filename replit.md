...
**November 8, 2025 (Latest)** - **Implemented customizable dashboard for project overview pages**. Added per-project dashboard customization feature allowing users to personalize the 2x2 widget grid on project overview pages. Users can select from 4 widgets (Basic Information, Financial Progress, Progress Overview, Action Points) for each quadrant via "Customize Dashboard" button. Changes persist to database (projects.dashboard_layout JSON column) and survive page reloads. Added PATCH /api/projects/:id/dashboard-layout endpoint with validation. Created widget registry architecture with self-contained widget components. Fixed cache invalidation to properly update UI after save. Default layout: Basic Info (top-left), Financial Progress (top-right), Progress Overview (bottom-left), Action Points (bottom-right).
